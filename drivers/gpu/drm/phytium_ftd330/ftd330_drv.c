...
static int ftd330_do_suspend(struct device *dev,  bool is_s3)
{
	int ret = 0;
	struct ftd330_dc *dc = dev_get_drvdata(dev);
...
static int ftd330_drm_suspend(struct device *dev)
{
	...
	return ftd330_do_suspend(dev, is_s3);
}

static int ftd330_drm_freeze(struct device *dev)
{
	bool is_s3 = 0;
...
static int ftd330_drm_poweroff(struct device *dev)
{
	bool is_s3 = 0;
...
static const struct dev_pm_ops ftd330_drm_pm_ops = {
	...
	.freeze = ftd330_drm_freeze,
	.thaw = ftd330_drm_thaw,
	.poweroff = ftd330_drm_poweroff,
	.restore = ftd330_drm_restore,
};
#endif

void phytium_display_disable_before_init(struct ftd330_drm_private *priv)
{
        struct platform_device *pdev = priv->pdev;
        struct device *dev = &pdev->dev;
        struct ftd330_dc *dc = dev_get_drvdata(dev);
...
static int ftd330_drm_platform_probe(struct platform_device *pdev)
{
	...
	/*we need disable all display before init*/
	phytium_display_disable_before_init(priv);
	...
	if (priv->info.pipe_mask & BIT(DISPLAY_0)) {
		ret = request_irq(platform_get_irq(pdev, 0),
						  dc0_isr, IRQF_SHARED, dev_name(dev), dev_get_drvdata(priv->dc_dev));
		if (ret < 0) {
	...
	if (priv->info.pipe_mask & BIT(DISPLAY_1) || priv->info.pipe_mask & BIT(DISPLAY_2)) {
		ret = request_irq(platform_get_irq(pdev, 1),
						  dc1_isr, IRQF_SHARED, dev_name(dev), dev_get_drvdata(priv->dc_dev));
		if (ret < 0) {
			dev_err(dev, "Failed to install irq\n");
			return ret;
		}
