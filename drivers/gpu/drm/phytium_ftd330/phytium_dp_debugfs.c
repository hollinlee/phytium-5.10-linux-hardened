...
#ifndef CONFIG_PHYTIUM_DCDP_EMULATOR
	phy_power_staus = ftd330_dp_phy_power_is_A0_state(phytium_dp);
	seq_printf(m, "PHY power is %s\n", phy_power_staus ? "on" : "off");
#endif
	return 0;
}
...
static int phytium_vrr_state_show(struct seq_file *m, void *data)
{
	...
	val = phytium_dplp_read_reg(priv, dplp_offset, PHYTIUM_DPLP_FRAME_COMP_CFG);
	if (val & LOWFPS_FUNC_EN)
		seq_puts(m, "Currently in lowfps auto mode\n");
	else
		seq_puts(m, "Currently in lowfps manual mode\n");
...
static int phytium_psr_state_show(struct seq_file *m, void *data)
{
	...
	seq_printf(m, "sink %sneed train on exit\n",
			phytium_dp->psr.sink_no_train_on_exit ? "does not " : "");
	seq_printf(m, "sink setup time is %d us\n",
			phytium_dp->psr.sink_setup_time);
	seq_printf(m, "sink sync latency needs %d frames, duration is %d ms\n",
			phytium_dp->psr.sink_sync_latency,
			phytium_dp->psr.sink_sync_latency * (1000 / phytium_dp->freq));
...
static int phytium_dp_force_training_show(struct seq_file *m, void *data)
{
	struct drm_connector *connector = m->private;
	struct phytium_dp_device *phytium_dp = connector_to_dp_device(connector);
	struct drm_device *dev =  phytium_dp->dev;
	...
	if (connector->status == connector_status_disconnected)
		return 0;
	phytium_dp_start_link_train(phytium_dp, true);
	mdelay(2);
...
static int phytium_dp_force_training_open(struct inode *inode, struct file *file)
{
	return single_open(file, phytium_dp_force_training_show, inode->i_private);
}

static const struct file_operations phytium_dp_force_training = {
	.owner = THIS_MODULE,
	.open = phytium_dp_force_training_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
#endif
static int phytium_dp_force_panel_dpms_show(struct seq_file *m, void *data)
{
	struct drm_connector *connector = m->private;
	struct phytium_dp_device *phytium_dp = connector_to_dp_device(connector);
#ifdef CONFIG_PHYTIUM_LANE_TRAIN
...
static int phytium_dp_force_panel_dpms_open(struct inode *inode, struct file *file)
{
	return single_open(file, phytium_dp_force_panel_dpms_show, inode->i_private);
}

static const struct file_operations phytium_force_panel_dpms = {
	.owner = THIS_MODULE,
	.open = phytium_dp_force_panel_dpms_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
#ifdef CONFIG_PHYTIUM_LANE_TRAIN
	{"force_training", &phytium_dp_force_training},
#endif
	{"force_panel_dpms", &phytium_force_panel_dpms},
};
