...
static s32 YUV601_2RGB_LIMITED[YUV_TO_RGB_TABLE_SIZE] = {
	1196, 0, 1638, 1196, -402, -834, 1196, 2070, 0, -914688, 556800, -1135872, 64, 940, 64, 960
};
...
static s32 YUV709_2RGB_LIMITED[YUV_TO_RGB_TABLE_SIZE] = {
	1196, 0, 1839, 1196, -219, -547, 1196, 2167, 0, -1017600, 316160, -1185536, 64, 940, 64, 960
};
...
static s32 YUV2020_2RGB_LIMITED[YUV_TO_RGB_TABLE_SIZE] = {
	1196, 0, 1722, 1196, -192, -667, 1196, 2197, 0, -957696, 363776, -1200896, 64, 940, 64, 960
...
static s32 YUV601_2RGB_FULL[YUV_TO_RGB_TABLE_SIZE] = { 1024, 0,	   1434, 1024,	  -352,	  -731,
						       1024, 1813, 0,	 -733696, 555008, -927744 };

static s32 YUV709_2RGB_FULL[YUV_TO_RGB_TABLE_SIZE] = { 1024, 0,	   1611, 1024,	  -192,	  -479,
						       1024, 1898, 0,	 -824320, 344064, -971264 };
...
static s32 YUV2020_2RGB_FULL[YUV_TO_RGB_TABLE_SIZE] = {
	1024, 0, 1509, 1024, -168, -584, 1024, 1925, 0, -772096, 385536, -985088
};
...
int dc_hw_get_interrupt(struct dc_hw *hw, struct dc_hw_interrupt_status *status, u32 dc_id)
{
	...
	if (intr_status & BIT(0)) {
		...
		drm_crtc = drm_crtc_from_index(drm_dev, display_id);
		if (drm_crtc) {
			ftd330_crtc = to_ftd330_crtc(drm_crtc);
			if (ftd330_crtc) {
...
{
        u32 dc_offset = 0;
	u32 reg = DC_DISPLAY_PANEL_START; 

	dc_write(hw, reg + dc_offset, 0);
	dc_write(hw, reg + dc_offset + DC_REG_OFFSET, 0);
