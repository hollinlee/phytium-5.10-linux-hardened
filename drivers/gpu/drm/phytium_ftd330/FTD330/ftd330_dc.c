...
static u32 ftd330_dc_get_vblank_count(struct ftd330_crtc *crtc)
{
	...
	struct drm_vblank_crtc *vblank = &drm_dev->vblank[crtc->id];
	int kernel_vblank_count = 0;
	int driver_vblank_count = dc_hw_get_vblank_count(&dc->hw, crtc->id);
	u64 diff_ns;
	u32 diff;

	drm_crtc = &crtc->base;
	if (!drm_crtc) {
		pr_err("FTD330 get vblank count drm_crtc erron\n");
		goto out;
	}

	drm_dev = drm_crtc->dev;
	if (!drm_dev) {
		pr_err("FTD330 get vblank count drm_dev erron\n");
		goto out;
	}

	vblank = &drm_dev->vblank[crtc->id];
	if (!vblank) {
		pr_err("FTD330 get vblank count vblank erron\n");
		goto out;
	}

	kernel_vblank_count = vblank->last;

	diff_ns = ktime_to_ns(ktime_sub(crtc->t_vblank, vblank->time));
	diff = DIV_ROUND_CLOSEST_ULL(diff_ns, vblank->framedur_ns);
	if (diff <= 0) {
		if (kernel_vblank_count != driver_vblank_count) {
			return kernel_vblank_count;
		}
	}

out:
	return driver_vblank_count;
}
...
static void update_cursor_plane(struct ftd330_dc *dc, struct ftd330_plane *plane)
{
	...
	if (dc_fake_mode_enable &&
		...
		if (cursor.x > mode->crtc_hdisplay)
			cursor.x = mode->crtc_hdisplay - 1;

		if (cursor.y < 0)
			cursor.y = 0;

                if (cursor.y > mode->crtc_vdisplay)
                        cursor.y = mode->crtc_vdisplay - 1;


		if (cursor.hot_x < 0)
			cursor.hot_x = 0;

		if (cursor.hot_x > state->crtc_w)
			cursor.hot_x = state->crtc_w - 1;

		if (cursor.hot_y < 0)
			cursor.hot_y = 0;

		if (cursor.hot_y > state->crtc_h)
			cursor.hot_y = state->crtc_h - 1;
	}
...
irqreturn_t dc0_isr(int irq, void *data)
{
...
irqreturn_t dc1_isr(int irq, void *data)
{
	struct ftd330_dc *dc = data;
	const struct ftd330_dc_info *dc_info = dc->hw.info;
	struct dc_hw_interrupt_status status = {0};
...
static int ftd330_dc_platform_init(struct drm_device *drm_dev, struct ftd330_dc *dc)
{
	...
	if (pdev->dev.of_node) {
		...
		if (ret < 0) {
			...
			priv->info.water_mark[0] = 0x5666;
			priv->info.water_mark[1] = 0x5666;
			priv->info.water_mark[2] = 0x5666;
		}
		...
		if (ret < 0) {
			...
			priv->info.qos[0] = 0xf0;
			priv->info.qos[1] = 0xf0;
		}
		...
		if (ret < 0) {
			...
			priv->info.overlay_enable = false;
		}
		...
		if (priv->info.edp_mask) {
			...
			if (ret < 0) {
				dev_info(&pdev->dev,
				"missing pwm freq property from dts, using default setting\n");
			...
			if (ret < 0) {
				dev_info(&pdev->dev,
					"missing edp_need_poweron from dts, poweron all cases\n");
			...
			ret =
			of_property_read_u8(node, "psr_enable",
									&bios_psr_is_enable);
			if (ret < 0) {
				dev_info(&pdev->dev,
					"missing bios_psr_enable from dts, default enable\n");
				bios_psr_is_enable = 1;
			} else {
				pr_info("FTD330 psr_enable is: %d\n", bios_psr_is_enable);
			}
			priv->info.bios_psr_is_enable = bios_psr_is_enable;
		}
#endif
		...
		if (priv->info.edp_mask) {
			...
			ret =
			fwnode_property_read_u8(np, "psr_enable",
									&bios_psr_is_enable);
			if (ret < 0) {
				dev_info(&pdev->dev,
					"missing bios_psr_enable from acpi, default enable\n");
				bios_psr_is_enable = 1;
			} else {
				pr_info("FTD330 psr_enable is: %d\n", bios_psr_is_enable);
			}
	...
	if (pdev->dev.of_node) {
		node = pdev->dev.of_node;
		ret = of_property_read_u32(node, "memory-region", &reserved_phandle);
		if (ret) {
			dev_err(&pdev->dev, "No memory-region specified\n");
			return ret;
		}

		reserved_mem_node = of_find_node_by_phandle(reserved_phandle);
		if (!reserved_mem_node) {
			dev_err(&pdev->dev,"Feiled to find node for phandle 0x%x\n", reserved_phandle);
			return -1;
		}

		ret = of_address_to_resource(reserved_mem_node, 0, &mem_res);
		of_node_put(reserved_mem_node);
		if (ret) {
			dev_err(&pdev->dev, "Failed to get resource from address\n");
			return ret;
		}

		priv->mem_pool_size = resource_size(&mem_res);
		priv->mem_pool_start_address_phy = mem_res.start;

		dev_info(&pdev->dev, "priv->mem_pool_start_address_phy addr: 0x%llx, size: 0x%llx\n",
				priv->mem_pool_start_address_phy, priv->mem_pool_size);
	} else {
		res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
		if (res) {
			priv->mem_pool_size = resource_size(res);
			priv->mem_pool_start_address_phy = res->start;
			dev_info(&pdev->dev, "Reserved memory phys addr: 0x%llx, size: 0x%llx\n",
				priv->mem_pool_start_address_phy, priv->mem_pool_size);
		} else {
	...
	if (pdev->dev.of_node) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 18)
		priv->se_regs = devm_platform_ioremap_resource(pdev, 1);
#else
	resour = platform_get_resource(pdev, IORESOURCE_MEM, 1);
