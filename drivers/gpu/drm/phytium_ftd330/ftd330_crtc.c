...
static ssize_t phytium_crtc_clear_mode_write(struct file *file, const char __user *ubuf, size_t len,
									loff_t *offp)
{
	struct seq_file *s = file->private_data;
	struct drm_crtc *crtc = s->private;
	...
	int enable = 0;
	unsigned int data = 0;
	u32 dc_offset = display_get_addr_offset(hw, ftd330_crtc->id);
	int ret;
	...
	buf[len] = '\0';
	ret = sscanf(buf, "%d %x", &enable, &data);
	if (ret != 2) {
		pr_info("Error: invalid input format, expected '<enable> <data>'\n");
		return -EINVAL;
	}

	if (enable) {
		dc_write(hw, DC_FRAMEBUFFER_CLEAR_VALUE + dc_offset, data);
		dc_set_clear(hw, DC_FRAMEBUFFER_CONFIG + dc_offset,
				(1 << 8), BIT(8));
	} else {
		dc_set_clear(hw, DC_FRAMEBUFFER_CONFIG + dc_offset,
				(0 << 8), BIT(8));
	}
	dc_set_clear(hw, DC_FRAMEBUFFER_CONFIG_EX + dc_offset, BIT(12), 0);

	return len;
}

static int phytium_crtc_clear_mode_show(struct seq_file *s, void *data)
{
	struct drm_crtc *crtc = s->private;
	struct ftd330_crtc *ftd330_crtc = to_ftd330_crtc(crtc);
	struct drm_device *drm_dev = crtc->dev;
	...
	value = readl(hw->reg_base + DC_FRAMEBUFFER_CONFIG + dc_offset);
	if (value & BIT(8))
		seq_puts(s, "clear mode is enabled\n");
	else
		seq_puts(s, "clear mode is disabled\n");
...
static int phytium_crtc_clear_mode_open(struct inode *inode, struct file *file)
{
	return single_open(file, phytium_crtc_clear_mode_show, inode->i_private);
}
...
static const struct file_operations phytium_crtc_clear_mode_fops = {
	.open = phytium_crtc_clear_mode_open,
	.write = phytium_crtc_clear_mode_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int ftd330_crtc_debugfs_init(struct drm_crtc *crtc)
{
	...
	debugfs_create_file("clear_mode", 0644, crtc->debugfs_entry, crtc, &phytium_crtc_clear_mode_fops);
	if (crtc_index == 0)
